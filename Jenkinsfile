 ...
 agent any
  ...
  post { 
	    ...
	    withCredentials([azureServicePrincipal('Azure_Prod_SPN')]) {
            ...
            sh 'az account show'
            sh 'az storage blob upload -f code_holder.tar.gz -c pyhtondatabricks -n code_holder.tar.gz --account-name pythondatabricks --account-key s6ZfN2talz87d9CAZIFOvrDY7DKEJw3dIII7Te264CopyL5XBiC2JUf+VAYEUa8ZbSQCE6bvu+X9808FhWZbJw'
        }
